...
abstract class Controller extends SRMController {
    ...
    const DONATE_PAYPAL = 'https://www.paypal.me/vojjov';
    const DONATE_YANDEX = 'https://money.yandex.ru/to/410012391377310';
                    ...
                    if (!$this->hasPreAction()) {
                        if ($this->isPreActionFileWritable()) {
                            ...
                            if ($this->hasPreAction()) {
                                ...
                                $this->error['pre_action_file'] = $this->language->get('error_pre_action_place_not_found');
                                $data['help'] = sprintf($this->language->get('help_pre_action_file_manual_insert_unknown_place'), $this->getPreActionFile());
                                $data['content'] = implode("\n", $this->getPreActionRegistrationLines());
                                $data['path'] = $this->getPreActionFile();
                                $status = false;
                            }
                        } else {
                            $this->error['pre_action_file'] = $this->language->get('error_pre_action_file_not_writable');
    ...
    protected function initRedirects() {
        ...
        if ($this->model_tool_simple_redirect_master) {
            ...
            $this->initPagination(
                $this->link($this->getRouteName(), $this->getUrlParameters(array('page')) . '&page={page}'),
    ...
    protected function initBreadcrumbs() {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href' => $this->link('extension/' . (version_compare(VERSION, '2.3.0.0', '<') ? 'module' : 'extension'))
        );

        $this->data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->link($this->getRouteName(), '')
        );
    }
    ...
    protected function initDeveloperInfo() {
        ...
        $this->data['donate'] = sprintf(
            $this->language->get('text_donate'),
            self::DONATE_PAYPAL,
            self::DONATE_YANDEX
        );
    }
     ...
     * @return string
     */
    protected function link($route, $args = '', $secure = null) {
        // Auto detect secure
        if (null === $secure)
            $secure = isset($this->request->server['HTTPS']) && (int)$this->request->server['HTTPS'];
     ...
     * @return array
     */
    protected function getPreActionRegistrationLines($empty_line = false, $indent = '') {
        $lines = array();
        ...
        $lines[] = '// Simple Redirect Master';

        if (version_compare(VERSION, '2.2.0.0', '>=')) {
            $lines[] = '\'startup/simple_redirect_master\',';
        } else {
            $lines[] = '$controller->addPreAction(new Action(\'startup/simple_redirect_master\'));';
        }

        $lines[] = '// End Simple Redirect Master';
        ...
        for ($i = 0, $total = count($lines); $i < $total; $i++) {
            ...
            if (version_compare(VERSION, '2.2.0.0', '>=')) {
                ...
                if ($preActionArrayStarted && preg_match('/^(\s*?)\'startup\/seo_url/', $line, $indent)) {
                    array_splice($lines, $i, 0, $this->getPreActionRegistrationLines(true, $indent[1]));
                    break;
                }
            } else {
                if (preg_match('/\$controller->addPreAction\s*\(\s*new\s+Action\(\s*\'common\/maintenance\'\s*\)\s*\)\s*;/', $line)) {
                    array_splice($lines, $i, 0, $this->getPreActionRegistrationLines(true));
                    break;
                }
            }
        }
